...
def wait_for_test_process(proc, timeout):
    ...
    while proc.returncode is None and time.time() < endtime:
        time.sleep(0.1)
        # os.read returns bytes in Python 3
    ...
    # Join bytes and decode to string for splitting
    lines = b''.join(err_chunks).decode('utf-8', errors='replace').split('\n')
...
def test_project1(binary):
    ...
    for filename in test_files:
      # Find the start of the filename (handling paths)
      try:
      ...
      while not done_testing:
        ...
        for lineIndex in range(0, len(lines)):
            ...
            # Manual increment for peeking ahead logic retained from original
            # Note: Changing lineIndex here does not affect the loop iterator
            lineIndex += 1
            ...
            if match:
                ...
                if not QUIET or (QUIET and not passed):
                    ...
                    # Use end='' to emulate trailing comma behavior
                    print(testname.ljust(64), end='')
                    print_result(passed)
                if not passed:
                    # Guard against IndexError if failure is on the last line
